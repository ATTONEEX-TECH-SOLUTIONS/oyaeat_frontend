  ...
  return (
    <div className="min-h-screen bg-[#f0f7f1] p-4 sm:p-6 pb-20">
       <div className="flex items-center justify-between mb-8">
          ...
          <button onClick={() => setNotifications(notifications.map(n => ({...n, read: true})))} className="bg-card border border-[#d8e4dc] px-4 py-2 rounded-xl text-sm font-bold text-[#2d6a4f] shadow-sm flex items-center gap-2 hover:bg-[#f0f7f3]">
       ...
       <div className="max-w-3xl space-y-3">
          {notifications.map(n => (
             <div key={n.id} className={`border border-[#d8e4dc] bg-card rounded-2xl p-5 shadow-sm flex gap-4 ${n.read ? 'opacity-60' : ''}`}>
