...
interface RidersListProps {
  riders: any[];
  onVerify?: (id: number, currentStatus: string) => Promise<void> | void;
  onReject?: (id: number) => Promise<void> | void;
  onDeactivate?: (id: number) => Promise<void> | void;
}
...
export function RidersList({
  ...
  // 🛡️ Guard against non-array or empty array properties safely
  if (!riders || !Array.isArray(riders) || riders.length === 0) {
    return (
      <div className="p-8 text-center bg-white border border-slate-200 rounded-xl text-slate-500 text-sm shadow-sm">
        No riders found matching this list selection.
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {riders.map((rider) => {
        ...
        const config =
          statusConfig[rider.status as keyof typeof statusConfig] ||
          ...
          >
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-slate-100 rounded-full flex items-center justify-center text-slate-700">
                      {(() => {
                        ...
                        const IconComponent =
                          VehicleIcon[type as keyof typeof VehicleIcon] || Bike;
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-500 capitalize">
                        {profile.vehicleType || "Unknown"} Partner{" "}
                  ...
                  <div className="mt-3 flex items-center gap-6 text-sm">
                    <span className="text-slate-600 flex items-center gap-1.5">
                      <Mail className="w-4 h-4 text-slate-400" /> {rider.email}
                    </span>
                    <span className="text-slate-600 flex items-center gap-1.5">
            ...
            {expandedId === rider.id && (
              <div className="border-t border-slate-200 p-6 bg-slate-50/70 space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-white p-4 rounded-xl border border-slate-200 flex flex-col justify-between shadow-sm">
                      <div className="flex items-start justify-between">
                        ...
                        <span
                          className={`text-xs px-2 py-0.5 rounded font-medium ${hasIdDocument ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"}`}
                        >
                      ...
                      <div className="mt-4 flex items-center justify-between">
                        <p className="text-xs text-slate-500 truncate max-w-[250px]">
                          {profile.idDocumentUrl
                            ? profile.idDocumentUrl.split("/").pop()
                    ...
                    <div className="bg-white p-4 rounded-xl border border-slate-200 flex flex-col justify-between shadow-sm">
                      <div>
                        <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block mb-1">
                          Plate Number Registry
                        </span>
                        <p className="text-xl font-mono font-bold text-slate-800">
                          {profile.plateNumber || "UNREGISTERED"}
                        </p>
                      </div>
                      <span className="text-xs text-slate-400">
                        Assigned for vehicle type:{" "}
                        <b className="capitalize text-slate-600">
                          {profile.vehicleType}
                        </b>
                      </span>
                    </div>
                ...
                {rider.status === "pending" && !hasIdDocument && (
                  <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 flex gap-3">
                    ...
                    <p className="text-sm text-yellow-800">
                      Cannot verify rider until the identification legal
                ...
                {(rider.status === "pending" ||
                  ...
                  <div className="flex gap-3 pt-2">
                    {hasIdDocument && rider.status === "pending" && (
                      <Button
                        onClick={() => onVerify?.(rider.id, "pending")}
                        className="flex-1 bg-green-600 hover:bg-green-700 text-white"
                      >
                        <Check className="w-4 h-4 mr-2" /> Verify Profiles &
                        Documents
                      </Button>
                    )}
                    {rider.status === "verified" && (
                      <Button
                        onClick={() => onVerify?.(rider.id, "verified")}
                        className="flex-1 bg-blue-600 hover:bg-blue-700 text-white"
                      >
                        <Check className="w-4 h-4 mr-2" /> Activate Rider
                        Account
                      </Button>
                    )}
                    {rider.status === "pending" && (
                ...
                {/* Existing Active profile deactivation block */}
                {rider.status === "active" && (
                  <div className="flex gap-3 pt-2">
                    <Button
                    ...
                    >
                      <X className="w-4 h-4 mr-2" /> Deactivate Account
                      Permissions
                    </Button>
                  </div>
                )}

                {/* Show reactivation option when account is inactive */}
                {rider.status === "inactive" && (
                  <div className="flex gap-3 pt-2">
                    <Button
                      onClick={() => onVerify?.(rider.id, "inactive")}
                      className="flex-1 bg-green-600 hover:bg-green-700 text-white"
                    >
                      <Check className="w-4 h-4 mr-2" /> Reactivate Rider
                      Permissions & Access
                    </Button>
                  </div>
                )}
